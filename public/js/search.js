...
const limit = 50;
const URLTenor =  `${window.location.origin}/get-more?q=${search}&limit=${limit}&`;
const URLGifsy = `${window.location.origin}/searchdb?search=${search}&limit=${limit}&`;
let URL;
let loading = false;
let next;//4
let db_ids = 0;
...
function loadDB(){
    ...
    .then(result=>{
        result.forEach((gif)=>{
            ...
            const img = document.createElement("img");
            const gifUrl = `${window.location.origin}/gif/${gif.name}`;
            img.setAttribute("src",gifUrl);
            img.className="rounded img-thumbnail";
            // img.style="max-width:300px; margin-right:10px";
            a.appendChild(img);
            a.href=`${window.location.origin}/show/gif/${gif.name}`;
            div.appendChild(a);
            gifContainer.appendChild(div);
